...
export default class FormDocumentEditView extends JetView {
  ...
  renderForm(elementsCount) {
    ...
    if (state.formData.data.configForm != false) {
      ...
      let table = this.$$("winEdit").queryView({"view": "datatable"});//this.$$('form_edit_data');
      if (state.isUpdate && table) {
        table.parse(state.table.data.getBranch(state.tableRecord.id));
      }
