...
export default class OrderUpholsteryView extends JetView{
  ...
  config(){
    ...
    return {
      ...
      rows:[
        {
          cols:[
            {"view": "label", width: 200, height:30, template: "План Обивка", css: { 'font-size':'17px', 'padding': '10px 0px 10px 15px', 'font-weight': 600}},
            {
        ...
        /*wjet::Settings*/
        {
          ...
          columns:[
            ...
            { id:"G",
              ...
              "css": {"color": "black", "text-align": "right",  "font-weight": 500}, editor:"text",  batch:1
            ...
            { id:"time_upholstery", header:[ "Время.об.план,ч", { content:"textFilter" }, { content:"totalColumn" } ],
            ...
            {
              id:"date_obivka",
              header:[ "Дата Об.факт", { content:"selectFilter" }, "" ],
              width:110,
              editor:"date",
              //format:webix.Date.dateToStr("%d.%m.%y"),
              batch:1,
...

            {
              ...
              header:[ "Дата Об.план", { content:"selectFilter" }, "" ],
              width:110,
              editor:"date",
              //format:webix.Date.dateToStr("%d.%m.%y"),
              batch:1,
            ...
            { id:"BA", header:[ "Статус Ст.", { content:"selectFilter" }, "" ], width:50, batch:1, editor:"text",
              "css": {"color": "green", "text-align": "center",  "font-weight": 500},
              template: function(obj) {
                if (obj.$group) return "";
                if (obj.BA == 1) {
            ...
            { id:"Z", header:[ "ФИО Об.", { content:"selectFilter" }, "" ], width:100, editor:"text", batch:1 },


            // { id:"coef_upholstery", header:[ "Коэф. Пош. гот.", { content:"textFilter" }, { content:"totalColumn" } ],
            //   width:120,
            ...
            { id:"desc_upholstery", header:[ "Причина", { content:"selectFilter" },""], width:150,  editor:"popup" },
            //{ id:"W", header:"Статус", width:100, batch:3 },
            //{ id:"AH", header:"Дата", width:100, batch:3 },
            { id:"loss_upholstery", header:[ "Классификация потерь", { content:"selectFilter" },""], width:110, batch:1, editor:"text" },
            { id:"time_loss_upholstery", header:[ "Время потерь, мин", { content:"selectFilter" },""], width:110, batch:1, editor:"text" },
          ...
          scheme:{
            $group:{
              ...
              map:{
                ...
                AG:["AG","median"],
                CH:["CH","median"],
               // coef_upholstery:["coef_upholstery", "median" ],
                //time_upholstery:["time_upholstery", "median" ],
                BO:["BO", "countSame" ],
                missing:false,
                value: [function (obj) {
                  return obj.AE;
            ...
            //   //row:"A"
            // },
            // $sort:{ by:"date_upholstery", dir:"asc", as: "date" },
        ...
        //     $init:function(obj){
        //       //obj.$css = obj.value;
        //       obj.date = obj.date_upholstery_plan;
        //       obj.value = obj.A;
        //       obj.details = obj.I;
        //     }
        //   },
  ...
  init(view) {
    ...
    //table.clearAll();
    //table.load(tableUrl);
    let table = this.$$("upholstery-table");
  ...
  doPlanToggle(type)  {
    ...
    if (type == 2) {
      let tableUrl = this.restApi.getUrl('get',"accounting/orders", {
        "per-page": "500",
        sort: '[{"property":"date_upholstery_plan","direction":"ASC"}, {"property":"index","direction":"ASC"}]',
        filter: '{"date_upholstery_plan":{">=":"'+dateFromValue+'","<=":"'+dateToValue+'"}}',
        //filter: '{"AE":{">=":"'+dateToValue+'"}}'
      });
      this.restApi.getLoad(tableUrl).then(function(data){
        table.clearAll();
        ...
        table.group({
          by: function (obj) {
            return formatDateTime(obj.date_upholstery_plan);
          },
          map:{
            G:["G","median"],
            V:["V","median"],
            ...
            AG:["AG","median"],
            CH:["CH","median"],
            //coef_obivka:["coef_obivka", "median" ],
            //time_upholstery:["time_upholstery", "median" ],
            BO:["BO", "countSame" ],
            missing:false,
            value: [function (obj) {
        ...
        //table.sort("date_upholstery_plan", "asc", "date");
