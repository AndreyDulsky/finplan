...
export default class OrderShipmentView extends JetView{
  ...
  config(){
    ...
    return {
      rows:[
        {
          cols:[
            {"view": "label", width: 200, height:30, template: "План отгрузки", css: { 'font-size':'17px', 'padding': '10px 0px 10px 15px', 'font-weight': 600}},
            {
        ...
        /*wjet::Settings*/
        {
          ...
          columns:[
...

            {
              ...
              template:function(obj, common){
                ...
                if (obj.$level==1) return common.treetable(obj, common) + formatDateTime(obj.value);
            ...
            { id:"date_shipment_plan", header:[ "Дата план", { content:"textFilter" }, "" ], width:110, editor:"date",
              template: function(obj) {
                return formatDateTime(parserDateTime(obj.date_shipment_plan));
              }
            },
            ...
            { id:"city", header:[ "Город", { content:"selectFilter" }, "" ], width:150, editor:"text" },
          ...
          scheme:{
            $group:{
              by:function(obj){ return obj.date_shipment_plan}, // 'company' is the name of a data property
              map:{
                G:["G","median"],
                V:["V","median"],
                AO:["AO","median"],
          ...
          ready:function(){
            ...
            let state = webix.storage.local.get("order-shipment-table");
            if (state)
              this.setState(state);
            //this.openAll();
          },
          ...
          on: {
            "onColumnResize" : function() {
              ...
              webix.storage.local.put("order-shipment-table", this.getState());
            },
            ...
            onBeforeDrop:function(context, e){
              let record = this.getItem(context.order-shipment);
  ...
  showBatch(newv){
    this.$$("order-shipment-table").showColumnBatch(newv);
  }
  ...
  loadColumnsSetting(columnsSetting, id) {
    ...
    this.configColumns.forEach((element, index) => {
      icon = iconEyeSlash;
      if(this.$$("order-shipment-table").isColumnVisible(element.id)) {
        icon = iconEye;
      }
  ...
  columnSettingClick(scope, id) {
    ...
    let grid = this.$$("order-shipment-table");
