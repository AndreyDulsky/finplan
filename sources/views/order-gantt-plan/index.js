...
export default class OrderGanttPlanView extends JetView{
      ...
      rows:[
        {
          cols:[
            ...
            { view:"icon", icon: 'mdi mdi-delete', autowidth:true, click: () =>  this.doClickClearPlan()},
            ...
            { view:"select",
              ...
              { id:1, value:"По дате шв.план" },
              { id:4, value:"По дате обивки план" },
              //{ id:5, value:"По дате обивки.excel" },
              { id:8, value:"По дате крой план" },
              { id:10, value:"По дате столярка план" },
            ],
              width: 150,
              on:{
                onChange:function(newv){
  ...
  getDataGantt(data) {
    ...
    data.forEach(function callback(item, index, array) {
      ...
      fieldSubGroup = item.product_order_id;
      ...
      //if (checkSewing) tasks.data.push(scope.getItem('date_first_plan', item, index, groupSub, '#d33daf'));
      if (checkUpholstery) tasks.data.push(scope.getItem('date_upholstery', item, index, groupSub, '#d33daf'));
  ...
  getId(item) {
    return item.product_order_id+'-'+item.id+'-';
  }

  getFieldsTime(fieldGroup) {
    let fields;
    fields  = {
      'date_carpenter' : {'start' : 'date_carpenter', 'end' : 'BD', 'time' : 'time_carpenter', 'name' : 'Столярка план', 'fio' : 'carpenter_employee',
        'plan' : 'date_carpenter_plan', 'status' : 'status_carpenter'
      },

      'date_sewing' : {'start' : 'date_sewing', 'end' : 'BU', 'time' : 'time_sewing', 'name' : 'Швека план', 'fio' : 'sewing_employee',
        'status' : 'status_sewing'},

      'date_cut' : {'start' : 'date_cut', 'end' : 'CA', 'time' : 'time_cut', 'name' : 'Крой план', 'fio' : 'cut_employee',
        'plan' : 'date_cut_plan', 'status' : 'status_cut' },

      'date_upholstery' : {'start' : 'date_upholstery', 'end' : 'date_upholstery_plan_end', 'time' : 'time_upholstery', 'name' : 'Обивка план',
        'fio' : 'upholstery_employee', 'status' : 'status_upholstery' },
      'date_first_plan' : {'start' : 'date_first_plan', 'end' : 'date_upholstery_plan_end', 'time' : 'time_upholstery', 'name' : 'Обивка excel',
        'fio' : 'upholstery_employee', 'status' : 'status_upholstery' },
  ...
  getItemGroup(field, item, timeStart, duration, index, parent, color) {
    ...
    timeStart = formatDateGantDay(item[times.start])+' 08:00';
  ...
  getData() {
    ...
    let tableUrl = this.app.config.apiRest.getUrl('get',"accounting/produce-plans",{
      "per-page": "500",
    ...
    webix.ajax().get(tableUrl).then(function(data){
      ...
      scope.doTableGroup();
      let tasks = scope.getDataGantt(data.json().data);
  ...
  init(view) {
    ...
    gantt.attachEvent("onAfterTaskDrag", function(id, mode, e){
      ...
      if(mode == modes.move ){
        ...
        //if (ids[2] == 'date_sewing' || ids[2] == 'date_cut' || ids[2] == 'AE' || ids[2] == 'date_upholstery' || ids[2] == 'date_carpenter') return;

        data[ids[2]] = formatDateTimeDb(item.start_date);


        let tableUrl = scope.app.config.apiRest.getUrl('put',"accounting/produce-plans", {}, item.db_id);
        webix.ajax().put(tableUrl, data).then(function(data){
          webix.message('Данные сохранены!');
        });
      }
  ...
  getFilterFieldByTypeGroup() {
    ...
    if (toggle.getValue() == 4) {
      field = 'date_upholstery';
    }
    if (toggle.getValue() == 5) {
      field = 'date_first_plan';
    }
