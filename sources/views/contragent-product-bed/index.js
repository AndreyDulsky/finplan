...
export default class ContragentProductsBedView extends JetView{
  config(){
    let scope = this;
    return {
      localId: "layout",
      ...
      cols:[
        {
          rows: [
...

            {
              ...
              urlEdit: 'contragent-product-bed',
              //autoConfig: true,
              ...
              columns:[
                ...
                { id:"product_id", header:"Модель", width: 280, sort: "string", collection: scope.app.config.apiRest.getCollection('accounting/product-beds',{'per-page': -1})
                },
                { id:"client_id", header:"Клиент",	width:120
                },
              ...
              url: this.app.config.apiRest.getUrl('get',"accounting/contragent-product-beds", {'sort':'name', 'per-page': -1, 'expand':'images'}),//"api->accounting/contragents",
              save: "api->accounting/contragent-product-beds",
              // scheme: {
  ...
  getData() {
    ...
    webix.ajax().get( scope.app.config.apiRest.getUrl('get','accounting/contragent-product-beds', {'sort':'name', 'per-page': -1}), objFilter).then(function(data) {
      table.parse(data);
      table.enable();
      table.openAll();
    });
